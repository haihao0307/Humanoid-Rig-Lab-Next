...
当前 `smpl-male-surface-skinned.glb` 仍使用原 24 骨调色板。新增骨骼已经进入编辑、姿势、动画、重定向和导出拓扑，但扭转、手指、眼睛、下颌和肩胛若要直接产生精细表皮形变，仍需要新的 SkinBinding 资产或重新绑定权重。这个限制不会影响现有 24 骨表皮和旧动作。

## 验证

自动验证覆盖 89 节点数量与角色、完整局部轴、镜像关系、VRM 手指命名、18 条重定向链、固定骨长、人体关节范围、轻量手指链、图片姿势重定向、动画局部四元数往返、GLB 骨架动画导出，以及视口分层显示代码。视觉效果由人工在本地浏览器验收。
