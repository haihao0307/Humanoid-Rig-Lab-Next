...
源码检查发现，`Agent.saveSafe/fail` 保存并恢复了骨架和 MotionController 内核，但没有恢复内核之外的连续步态相位、转身速度/加速度、平衡反馈的上一帧根位置和速度。候选姿态被拒绝后，这些过滤器仍停留在失败帧；恢复动作时可能出现相位、转向或加速度反馈不连续。后续授权运行的 CPU 回归已验证三种情形的恢复与确定性重放；没有量化修复前的视觉突变。
...
- 运行验证：用户要求按计划开始处理后，执行并通过新增回滚测试（三种情形、18 步重放），以及 phase/turn/support/standing-height/light-balance、任务 traffic/target-reservations 等回归。物理和可见提交使用桩的范围没有变化。
- 后续脚掌姿态、平衡预检和浏览器验证见 `MOTION_NATURALNESS_20260916.md`；运行结果与视觉验收分开记录。
...
- [x] 没有用生成图片代替真实三维实现。
- [x] 已实际修改生产源码。
- [x] 用户看到的是可交互三维工作台（后续同分支已重建并运行入口，见自然度记录）。
- [x] 人物/动物几何、骨骼和动作来自真实运行时（后续同分支已进行浏览器验证）。
- [x] 镜头、选择、动作或参数控制可以实际操作（后续验证了镜头、单人动作与六人分发）。
- [ ] 公网固定链接和真实浏览器已验证（仅验证本地真实浏览器，未发布公网固定地址）。
