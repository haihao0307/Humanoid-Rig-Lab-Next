...
const report = {
  schema: 'cat_kaopu/procedural_cat_browser_qa@1.0',
  buildId: 'cat-procedural-body-v1-p0-20260917',
  url,
  ready: null,
  pageErrors: [],
  consoleErrors: [],
...
try {
  ...
  await page.waitForFunction(() => window.__CAT_PROCEDURAL_READY__ !== undefined, null, { timeout: 120_000 });
  ...
  const capture = async (view) => {
    ...
    const filename = `CAT_PROCEDURAL_V1_P0_${view.toUpperCase().replaceAll('-', '_')}_2026-09-17.png`;
    const output = path.join(qaDir, filename);
    const buffer = await stage.screenshot({ path: output });
    report.screenshots[view] = {
  ...
  variantDna.meta.id = 'qa-long-leg-variant';
  variantDna.meta.revision += 1;
  variantDna.global.shoulderHeight *= 1.08;
  variantDna.global.hipHeight *= 1.06;
  variantDna.torso.lumbarWidth *= 0.94;
  variantDna.head.earHeight *= 1.08;
  ...
  const variantPath = path.join(qaDir, 'CAT_PROCEDURAL_V1_P0_VARIANT_2026-09-17.png');
  ...
  report.assertions.fixedViewsDistinct = new Set(Object.values(report.screenshots).slice(0, 6).map((item) => item.sha256)).size >= 5;
  ...
  await mobile.waitForFunction(() => window.__CAT_PROCEDURAL_READY__ === 'webgl2', null, { timeout: 120_000 });
  const mobilePath = path.join(qaDir, 'CAT_PROCEDURAL_V1_P0_MOBILE_390x844_2026-09-17.png');
  ...
  await browser.close();
  fs.writeFileSync(path.join(qaDir, 'CAT_PROCEDURAL_V1_P0_BROWSER_QA_2026-09-17.json'), `${JSON.stringify(report, null, 2)}\n`, 'utf8');
}
