...
precision highp float;layout(location=0)in vec4 aParam;layout(location=1)in vec3 aCenter;layout(location=2)in vec3 aRadius;layout(location=3)in float aRim;layout(location=4)in vec3 aOuterPos;layout(location=5)in vec3 aOuterNor;uniform mat4 uPV;uniform mat4 uHead;uniform float uBlink;uniform float uThicknessM;out vec3 vN;out float vRim;out float vLid;out float vClosure;out float vBand;float smooth01(float x){x=clamp(x,0.,1.);return x*x*(3.-2.*x);}void main(){float ex=aParam.x,t=aParam.y,lid=aParam.z;float hx=clamp(ex/.995,-1.,1.),q=sqrt(max(0.,1.-hx*hx)),apx=hx/.52,apq=sqrt(max(0.,1.-apx*apx)),side=sign(aCenter.y),lateral=hx*side,medial=smooth01((-lateral-.2)/.8),arc=pow(apq,mix(.82,1.08,medial)),corner=-.025+.070*lateral+.012*(1.-q),upperOpen=corner+(.34-.02*lateral)*arc,lowerOpen=corner-(.28+.01*lateral)*arc,seamLine=corner-(.035-.010*lateral)*arc,close=smoothstep(0.,1.,clamp(uBlink,0.,1.)),openInner=lid>0.?upperOpen:lowerOpen,lidClose=lid>0.?close:pow(close,1.22),inner=mix(openInner,seamLine,lidClose),innerEf=sqrt(max(.0008,1.-hx*hx-inner*inner)),rMean=max(.0001,(aRadius.x+aRadius.y+aRadius.z)/3.),frontScale=1.+uThicknessM/rMean;vec3 innerDir=normalize(vec3(innerEf,hx,inner)),innerP=aCenter+aRadius*innerDir*frontScale,innerN=normalize(vec3(innerDir.x/max(aRadius.x,.0001),innerDir.y/max(aRadius.y,.0001),innerDir.z/max(aRadius.z,.0001)));float tt=smoothstep(0.,1.,t),canthusBlend=smoothstep(.02,.24,apq),bandBulge=4.*tt*(1.-tt)*canthusBlend;vec3 closeNormal=normalize(vec3(1.,hx*.10,inner*.08));innerN=normalize(mix(innerN,closeNormal,close*(1.-tt)*.28));vec3 p,n;if(aParam.w<.5){vec3 outerN=normalize(aOuterNor),outerP=aOuterPos+outerN*uThicknessM*.12;p=mix(innerP,outerP,tt);p=mix(outerP,p,canthusBlend);vec3 sphereLocal=(p-aCenter)/aRadius;float sphereR2=sphereLocal.y*sphereLocal.y+sphereLocal.z*sphereLocal.z,sphereInside=1.-step(1.,sphereR2),sphereSafeX=aCenter.x+aRadius.x*sqrt(max(.0008,1.-sphereR2))*frontScale,sphereSafety=(1.-smoothstep(.90,.995,tt))*sphereInside;p.x=mix(p.x,max(p.x,sphereSafeX),sphereSafety);p.x+=close*(lid>0.?uThicknessM*.34:uThicknessM*.18)*(1.-tt)*canthusBlend;p.x+=close*bandBulge*aRadius.x*(lid>0.?.115:.018);n=normalize(mix(innerN,outerN,tt));n=normalize(mix(outerN,n,canthusBlend));}else{float shell=aParam.w<1.5?frontScale:1.-uThicknessM/rMean*.35;p=aCenter+aRadius*innerDir*shell;p=mix(aOuterPos,p,canthusBlend);p.x+=close*(lid>0.?uThicknessM*.34:uThicknessM*.18)*canthusBlend;vec3 rimNormal=normalize(vec3(.42,0.,-lid*.91));n=normalize(mix(innerN,rimNormal,step(.5,aRim)));n=normalize(mix(aOuterNor,n,canthusBlend));}vec4 wp=uHead*vec4(p,1.);gl_Position=uPV*wp;vN=normalize(mat3(uHead)*n);vRim=aRim;vLid=lid;vClosure=close;vBand=t;}`;
    const lidFS=`#version 300 es
    precision highp float;in vec3 vN;in float vRim;in float vLid;in float vClosure;in float vBand;uniform float uDebug;out vec4 outColor;void main(){vec3 n=normalize(vN),l1=normalize(vec3(.45,-.55,.82)),l2=normalize(vec3(-.62,.35,.35));float d=max(dot(n,l1),0.)*.70+max(dot(n,l2),0.)*.24+.19;vec3 base=vec3(.69,.655,.59);float rim=smoothstep(.15,.85,vRim);float closedGate=smoothstep(.72,.98,vClosure);float seamBand=(1.-smoothstep(.012,.100,vBand))*closedGate;float seamDark=seamBand*step(0.,vLid);float foldBand=(smoothstep(.10,.17,vBand)-smoothstep(.20,.30,vBand))*closedGate*step(0.,vLid);float edgeAlpha=1.-smoothstep(.94,1.,vBand);base*=vLid>0.?.965:.985;base*=mix(1.,.60,rim);base*=1.-.14*rim*smoothstep(.45,.95,vClosure);base*=1.-.64*seamDark;base*=1.-.13*foldBand;float soft=pow(1.-max(dot(n,normalize(vec3(.2,-.4,.9))),0.),2.);vec3 col=base*d+vec3(.025,.04,.045)*soft;if(uDebug>.5)col=mix(vec3(.08,.52,.92),vec3(1.,.30,.07),step(0.,vLid))*(.55+.45*max(dot(n,l1),0.));outColor=vec4(pow(max(col,vec3(0.)),vec3(1./2.2)),uDebug>.5?1.:edgeAlpha);}`;
...
function eyeCurves(spec,ex){const hx=Math.max(-1,Math.min(1,ex/.995)),q=Math.sqrt(Math.max(0,1-hx*hx)),lateral=hx*Math.sign(spec.center[1]||1),medial=smooth01((-lateral-.2)/.8),arc=Math.pow(Math.max(q,.0001),.78*(1-medial)+1.12*medial),corner=-.025+.070*lateral+.012*(1-q);return{hx,q,lateral,arc,corner,upper:corner+(.36-.02*lateral)*arc,lower:corner-(.17+.01*lateral)*arc,seam:corner-(.055-.015*lateral)*arc,outerUpper:corner+.62*arc,outerLower:corner-.31*arc}}
...
for(const spec of specs)for(const lid of[1,-1]){const outer=[];for(let i=0;i<=cols;i++){const ex=-xmax+2*xmax*i/cols,c=eyeCurves(spec,ex),ey=lid>0?c.outerUpper:c.outerLower,y=spec.center[1]+spec.radius[1]*c.hx,z=spec.center[2]+spec.radius[2]*ey;outer.push(sampleFace(spec,y,z,c.hx,ey))}const base=vcount;for(let i=0;i<=cols;i++){const ex=-xmax+2*xmax*i/cols;for(let j=0;j<=rows;j++)push(spec,ex,j/rows,lid,0,false,outer[i])}for(let i=0;i<cols;i++)for(let j=0;j<rows;j++){const a=base+i*(rows+1)+j,b=a+rows+1;idx.push(a,b,a+1,a+1,b,b+1)}const rb=vcount;for(let i=0;i<=cols;i++){const ex=-xmax+2*xmax*i/cols;push(spec,ex,0,lid,1,true,outer[i]);push(spec,ex,0,lid,2,true,outer[i])}for(let i=0;i<cols;i++){const a=rb+i*2,b=a+2;idx.push(a,b,a+1,a+1,b,b+1)}}return{param:new Float32Array(param),center:new Float32Array(center),radius:new Float32Array(radius),rim:new Float32Array(rim),outerPos:new Float32Array(outerPos),outerNor:new Float32Array(outerNor),idx:new Uint16Array(idx),vertices:vcount,triangles:idx.length/3,eyeSpecs:specs,fitMode:'head_surface_raycast_canthus_anchored'}}
    ...
    new_frame_draw="gl.uniform1f(U.eye,eyeLayerEnabled?1:0);gl.uniform1f(U.blink,expressionState.blink);gl.uniform1f(U.cornea,corneaLayerEnabled?corneaResponse:0);gl.uniform1f(U.pupil,pupilAdapt);gl.uniform1f(U.fur,furLayerEnabled?1:0);gl.uniform1f(U.furStrength,furStrength);gl.uniform1f(U.furDebug,furDebugEnabled?1:0);gl.drawElements(gl.TRIANGLES,m.ic,gl.UNSIGNED_SHORT,0);gl.bindVertexArray(null);if(blinkLayerEnabled){gl.useProgram(lidPr);gl.bindVertexArray(lidVao);gl.uniformMatrix4fv(LidU.pv,false,pv);gl.uniformMatrix4fv(LidU.head,false,mats.skin.subarray(headBoneIndex*16,headBoneIndex*16+16));gl.uniform1f(LidU.blink,expressionState.blink);gl.uniform1f(LidU.thickness,lidThickness);gl.uniform1f(LidU.debug,lidDebugEnabled?1:0);gl.disable(gl.CULL_FACE);gl.enable(gl.BLEND);gl.blendFunc(gl.SRC_ALPHA,gl.ONE_MINUS_SRC_ALPHA);gl.drawElements(gl.TRIANGLES,lidMesh.idx.length,gl.UNSIGNED_SHORT,0);gl.disable(gl.BLEND);gl.enable(gl.CULL_FACE);gl.bindVertexArray(null)}drawLines(pv,mats.jpos);"
