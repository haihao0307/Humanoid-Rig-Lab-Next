...
def build_html(source_text: str) -> str:
    ...
    precision highp float;layout(location=0)in vec4 aParam;layout(location=1)in vec3 aCenter;layout(location=2)in vec3 aRadius;layout(location=3)in float aRim;uniform mat4 uPV;uniform mat4 uHead;uniform float uBlink;uniform float uThicknessM;out vec3 vN;out float vRim;out float vLid;out float vClosure;void main(){float ex=aParam.x,t=aParam.y,lid=aParam.z;float q=sqrt(max(.0001,1.-ex*ex));float close=smoothstep(0.,1.,clamp(uBlink,0.,1.));float side=sign(aCenter.y),medial=ex*side;float openInner=lid>0.?((.39-.025*medial)*q):((-.16-.015*medial)*q);float seamCenter=(-.018+.030*medial)*q;float seam=seamCenter+(lid>0.?-.012:.012)*q;float lidClose=lid>0.?smoothstep(0.,1.,close):pow(close,1.18);float inner=mix(openInner,seam,lidClose);float outer=(lid>0.?.82:-.72)*q;float tt=smoothstep(0.,1.,t);float ey=mix(inner,outer,tt);float sphereEf=sqrt(max(.0001,1.-ex*ex-ey*ey));float flatten=.10*close*(1.-.62*tt)*q*q;float ef=max(.08,sphereEf-flatten);float rMean=max(.0001,(aRadius.x+aRadius.y+aRadius.z)/3.);float frontScale=1.+uThicknessM/rMean;float surfaceScale=mix(frontScale,.985,tt);float shell=aParam.w<.5?surfaceScale:(aParam.w<1.5?frontScale:.982);vec3 dir=vec3(ef,ex,ey);vec3 p=aCenter+aRadius*dir*shell;p.x+=close*(lid>0.?uThicknessM*.35:-uThicknessM*.15)*(1.-tt);vec3 radial=normalize(vec3(dir.x/max(aRadius.x,.0001),dir.y/max(aRadius.y,.0001),dir.z/max(aRadius.z,.0001)));vec3 rimNormal=normalize(vec3(.42,0.,-lid*.91));vec3 n=normalize(mix(radial,rimNormal,step(.5,aRim)));vec4 wp=uHead*vec4(p,1.);gl_Position=uPV*wp;vN=normalize(mat3(uHead)*n);vRim=aRim;vLid=lid;vClosure=close;}`;
    const lidFS=`#version 300 es
    precision highp float;in vec3 vN;in float vRim;in float vLid;in float vClosure;uniform float uDebug;out vec4 outColor;void main(){vec3 n=normalize(vN),l1=normalize(vec3(.45,-.55,.82)),l2=normalize(vec3(-.62,.35,.35));float d=.25+.64*max(dot(n,l1),0.)+.16*max(dot(n,l2),0.);vec3 base=vec3(.69,.655,.59);float rim=smoothstep(.15,.85,vRim);base*=mix(1.,.58,rim);base*=1.-.18*rim*smoothstep(.45,.95,vClosure);float soft=pow(1.-max(dot(n,normalize(vec3(.2,-.4,.9))),0.),2.);vec3 col=base*d+vec3(.018,.024,.026)*soft;if(uDebug>.5)col=mix(vec3(.08,.52,.92),vec3(1.,.30,.07),step(0.,vLid))*(.55+.45*max(dot(n,l1),0.));outColor=vec4(pow(max(col,vec3(0.)),vec3(1./2.2)),1.);}`;
    ...
    geom_code=r'''function buildGeometricEyelids(mesh){const groups=[[],[]];for(let i=0;i<mesh.vc;i++){if(mesh.part[i]>.5)groups[mesh.pos[i*3+1]>=0?0:1].push(i)}const specs=groups.map(ids=>{if(!ids.length)throw new Error('V4.42 眼球顶点缺失');const mn=[Infinity,Infinity,Infinity],mx=[-Infinity,-Infinity,-Infinity],c=[0,0,0];for(const i of ids)for(let k=0;k<3;k++){const v=mesh.pos[i*3+k];mn[k]=Math.min(mn[k],v);mx[k]=Math.max(mx[k],v);c[k]+=v}for(let k=0;k<3;k++)c[k]/=ids.length;return{center:c,radius:[(mx[0]-mn[0])*.5,(mx[1]-mn[1])*.5,(mx[2]-mn[2])*.5]}});const param=[],center=[],radius=[],rim=[],idx=[];let vcount=0;const cols=40,rows=6,xmax=.995;function push(spec,ex,t,lid,mode,isRim){param.push(ex,t,lid,mode);center.push(...spec.center);radius.push(...spec.radius);rim.push(isRim?1:0);return vcount++}for(const spec of specs)for(const lid of [1,-1]){const base=vcount;for(let i=0;i<=cols;i++){const ex=-xmax+2*xmax*i/cols;for(let j=0;j<=rows;j++)push(spec,ex,j/rows,lid,0,false)}for(let i=0;i<cols;i++)for(let j=0;j<rows;j++){const a=base+i*(rows+1)+j,b=a+rows+1;idx.push(a,b,a+1,a+1,b,b+1)}const rb=vcount;for(let i=0;i<=cols;i++){const ex=-xmax+2*xmax*i/cols;push(spec,ex,0,lid,1,true);push(spec,ex,0,lid,2,true)}for(let i=0;i<cols;i++){const a=rb+i*2,b=a+2;idx.push(a,b,a+1,a+1,b,b+1)}}return{param:new Float32Array(param),center:new Float32Array(center),radius:new Float32Array(radius),rim:new Float32Array(rim),idx:new Uint16Array(idx),vertices:vcount,triangles:idx.length/3,eyeSpecs:specs}}
    ...
    new="const U={pv:gl.getUniformLocation(pr,'uPV'),bones:gl.getUniformLocation(pr,'uBones[0]'),weight:gl.getUniformLocation(pr,'uWeightMode'),posture:gl.getUniformLocation(pr,'uPostureCorrection'),corrective:gl.getUniformLocation(pr,'uCorrectiveEnabled'),normalCorrective:gl.getUniformLocation(pr,'uNormalCorrectiveEnabled'),debug:gl.getUniformLocation(pr,'uCorrectiveDebug'),gaze:gl.getUniformLocation(pr,'uGaze'),eye:gl.getUniformLocation(pr,'uEyeEnabled'),cornea:gl.getUniformLocation(pr,'uCornea'),pupil:gl.getUniformLocation(pr,'uPupilAdapt'),fur:gl.getUniformLocation(pr,'uFurEnabled'),furStrength:gl.getUniformLocation(pr,'uFurStrength'),furDebug:gl.getUniformLocation(pr,'uFurDebug')},LU={pv:gl.getUniformLocation(lpr,'uPV'),color:gl.getUniformLocation(lpr,'uColor')};\nconst lidVao=gl.createVertexArray();gl.bindVertexArray(lidVao);const lidParamBuf=attr(0,lidMesh.param,4,gl.FLOAT),lidCenterBuf=attr(1,lidMesh.center,3,gl.FLOAT),lidRadiusBuf=attr(2,lidMesh.radius,3,gl.FLOAT),lidRimBuf=attr(3,lidMesh.rim,1,gl.FLOAT),lidIb=gl.createBuffer();gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER,lidIb);gl.bufferData(gl.ELEMENT_ARRAY_BUFFER,lidMesh.idx,gl.STATIC_DRAW);gl.bindVertexArray(null);const LidU={pv:gl.getUniformLocation(lidPr,'uPV'),head:gl.getUniformLocation(lidPr,'uHead'),blink:gl.getUniformLocation(lidPr,'uBlink'),thickness:gl.getUniformLocation(lidPr,'uThicknessM'),debug:gl.getUniformLocation(lidPr,'uDebug')};"
    ...
    text=rep(text,"window.__CAT_V442_BASELINE__={surface:'V4.32',weights:'V4.40',posture:'V4.39',locomotion:'V4.36',payload:'CATV440'};",
             "window.__CAT_V442_BASELINE__={surface:'V4.32',weights:'V4.40',posture:'V4.39',locomotion:'V4.36',eyeExpression:'V4.41',payload:'CATV440'};window.__CAT_V442_EYELID_GEOMETRY__={pieces:4,vertices:lidMesh.vertices,triangles:lidMesh.triangles,headBoneIndex,source:'procedural-bounded-shells'};",
