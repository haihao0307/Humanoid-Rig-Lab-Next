...
def build_html(source_text: str) -> str:
    ...
    precision highp float;layout(location=0)in vec4 aParam;layout(location=1)in vec3 aCenter;layout(location=2)in vec3 aRadius;layout(location=3)in float aRim;uniform mat4 uPV;uniform mat4 uHead;uniform float uBlink;uniform float uThicknessM;out vec3 vN;out float vRim;out float vLid;out float vClosure;out float vBand;void main(){float ex=aParam.x,t=aParam.y,lid=aParam.z;float q=sqrt(max(.0001,1.-ex*ex));float close=smoothstep(0.,1.,clamp(uBlink,0.,1.));float side=sign(aCenter.y),medial=ex*side;float openInner=lid>0.?((.39-.025*medial)*q):((-.16-.015*medial)*q);float seamCenter=(-.135+.060*medial+.025*(1.-q))*q;float seam=seamCenter+(lid>0.?-.012:.012)*q;float lidClose=lid>0.?smoothstep(0.,1.,close):pow(close,1.18);float inner=mix(openInner,seam,lidClose);float outer=(lid>0.?.96:-.90)*q;float tt=smoothstep(0.,1.,t);float ey=mix(inner,outer,tt);float ef=sqrt(max(.0001,1.-ex*ex-ey*ey));float rMean=max(.0001,(aRadius.x+aRadius.y+aRadius.z)/3.);float frontScale=1.+uThicknessM/rMean;float surfaceScale=mix(frontScale,1.014,tt);float shell=aParam.w<.5?surfaceScale:(aParam.w<1.5?frontScale:.998);vec3 dir=normalize(vec3(ef,ex,ey));vec3 p=aCenter+aRadius*dir*shell;p.x+=close*(lid>0.?uThicknessM*.46:-uThicknessM*.16)*(1.-tt);vec3 radial=normalize(vec3(dir.x/max(aRadius.x,.0001),dir.y/max(aRadius.y,.0001),dir.z/max(aRadius.z,.0001)));vec3 orbitNormal=normalize(vec3(.78,ex*.22,ey*.28));radial=normalize(mix(radial,orbitNormal,smoothstep(.45,1.,tt)*.72));vec3 closeNormal=normalize(vec3(1.,ex*.10,ey*.08));radial=normalize(mix(radial,closeNormal,close*(1.-tt)*.58));vec3 rimNormal=normalize(vec3(.42,0.,-lid*.91));vec3 n=normalize(mix(radial,rimNormal,step(.5,aRim)));vec4 wp=uHead*vec4(p,1.);gl_Position=uPV*wp;vN=normalize(mat3(uHead)*n);vRim=aRim;vLid=lid;vClosure=close;vBand=t;}`;
