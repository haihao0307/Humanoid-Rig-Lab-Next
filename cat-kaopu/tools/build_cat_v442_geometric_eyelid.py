...
def build_html(source_text: str) -> str:
    ...
    precision highp float;layout(location=0)in vec4 aParam;layout(location=1)in vec3 aCenter;layout(location=2)in vec3 aRadius;layout(location=3)in float aRim;uniform mat4 uPV;uniform mat4 uHead;uniform float uBlink;uniform float uThicknessM;out vec3 vN;out float vRim;out float vLid;out float vClosure;out float vBand;void main(){float ex=aParam.x,t=aParam.y,lid=aParam.z;float q=sqrt(max(.0001,1.-ex*ex));float close=smoothstep(0.,1.,clamp(uBlink,0.,1.));float side=sign(aCenter.y),medial=ex*side;float openInner=lid>0.?((.39-.025*medial)*q):((-.16-.015*medial)*q);float seamCenter=(-.018+.030*medial)*q;float seam=seamCenter+(lid>0.?-.012:.012)*q;float lidClose=lid>0.?smoothstep(0.,1.,close):pow(close,1.18);float inner=mix(openInner,seam,lidClose);float outer=(lid>0.?.82:-.72)*q;float tt=smoothstep(0.,1.,t);float ey=mix(inner,outer,tt);float ef=sqrt(max(.0001,1.-ex*ex-ey*ey));float rMean=max(.0001,(aRadius.x+aRadius.y+aRadius.z)/3.);float frontScale=1.+uThicknessM/rMean;float surfaceScale=mix(frontScale,1.001,tt);float shell=aParam.w<.5?surfaceScale:(aParam.w<1.5?frontScale:.998);vec3 dir=normalize(vec3(ef,ex,ey));vec3 p=aCenter+aRadius*dir*shell;p.x+=close*(lid>0.?uThicknessM*.46:-uThicknessM*.16)*(1.-tt);vec3 radial=normalize(vec3(dir.x/max(aRadius.x,.0001),dir.y/max(aRadius.y,.0001),dir.z/max(aRadius.z,.0001)));vec3 orbitNormal=normalize(vec3(.78,ex*.22,ey*.28));radial=normalize(mix(radial,orbitNormal,smoothstep(.45,1.,tt)*.72));vec3 rimNormal=normalize(vec3(.42,0.,-lid*.91));vec3 n=normalize(mix(radial,rimNormal,step(.5,aRim)));vec4 wp=uHead*vec4(p,1.);gl_Position=uPV*wp;vN=normalize(mat3(uHead)*n);vRim=aRim;vLid=lid;vClosure=close;vBand=t;}`;
    const lidFS=`#version 300 es
    precision highp float;in vec3 vN;in float vRim;in float vLid;in float vClosure;in float vBand;uniform float uDebug;out vec4 outColor;void main(){vec3 n=normalize(vN),l1=normalize(vec3(.45,-.55,.82)),l2=normalize(vec3(-.62,.35,.35));float d=.25+.64*max(dot(n,l1),0.)+.16*max(dot(n,l2),0.);vec3 base=vec3(.69,.655,.59);float rim=smoothstep(.15,.85,vRim);float closedGate=smoothstep(.72,.98,vClosure);float seamBand=(1.-smoothstep(.012,.090,vBand))*closedGate;float seamDark=seamBand*step(0.,vLid);float foldBand=(smoothstep(.09,.15,vBand)-smoothstep(.18,.27,vBand))*closedGate*step(0.,vLid);float edgeAlpha=1.-smoothstep(.76,1.,vBand);base*=vLid>0.?.975:1.015;base*=mix(1.,.60,rim);base*=1.-.15*rim*smoothstep(.45,.95,vClosure);base*=1.-.62*seamDark;base*=1.-.14*foldBand;float soft=pow(1.-max(dot(n,normalize(vec3(.2,-.4,.9))),0.),2.);vec3 col=base*d+vec3(.018,.024,.026)*soft;if(uDebug>.5)col=mix(vec3(.08,.52,.92),vec3(1.,.30,.07),step(0.,vLid))*(.55+.45*max(dot(n,l1),0.));outColor=vec4(pow(max(col,vec3(0.)),vec3(1./2.2)),uDebug>.5?1.:edgeAlpha);}`;
