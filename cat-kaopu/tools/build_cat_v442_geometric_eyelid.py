...
def build_html(source_text: str) -> str:
    ...
    precision highp float;in vec3 vN;in float vRim;in float vLid;in float vClosure;in float vBand;uniform float uDebug;out vec4 outColor;void main(){vec3 n=normalize(vN),l1=normalize(vec3(.45,-.55,.82)),l2=normalize(vec3(-.62,.35,.35));float d=.25+.64*max(dot(n,l1),0.)+.16*max(dot(n,l2),0.);vec3 base=vec3(.69,.655,.59);float rim=smoothstep(.15,.85,vRim);float closedGate=smoothstep(.72,.98,vClosure);float seamBand=(1.-smoothstep(.015,.075,vBand))*closedGate;float seamDark=seamBand*step(0.,vLid);base*=mix(1.,.58,rim);base*=1.-.18*rim*smoothstep(.45,.95,vClosure);base*=1.-.56*seamDark;float soft=pow(1.-max(dot(n,normalize(vec3(.2,-.4,.9))),0.),2.);vec3 col=base*d+vec3(.018,.024,.026)*soft;if(uDebug>.5)col=mix(vec3(.08,.52,.92),vec3(1.,.30,.07),step(0.,vLid))*(.55+.45*max(dot(n,l1),0.));outColor=vec4(pow(max(col,vec3(0.)),vec3(1./2.2)),1.);}`;
