...
const qaDir = path.join(moduleRoot, 'qa', 'body-bind-v1-r2');
...
const report = {
  schema: 'cat_kaopu/cat_body_bind_v1_r2_browser_qa@1.0',
  buildId: 'cat-body-bind-v1-r2-20260917',
  url,
...
try {
  ...
  for (const view of ['front', 'left', 'right', 'top', 'quarter-front', 'quarter-rear']) {
    ...
    await capture(view, `CAT_BODY_BIND_V1_R2_${view.toUpperCase().replaceAll('-', '_')}_2026-09-17.png`);
  }
  ...
  await capture('skeletonQuarter', 'CAT_BODY_BIND_V1_R2_SKELETON_QUARTER_2026-09-17.png');
  await page.evaluate(() => document.querySelector('#skeletonBtn')?.click());
  ...
  await page.evaluate(() => window.__CAT_BODY_BIND_V1_SET_VIEW__('quarter-front'));
  await capture('strongQuarter', 'CAT_BODY_BIND_V1_R2_STRONG_PROFILE_QUARTER_2026-09-17.png');
  ...
  const mobilePath = path.join(qaDir, 'CAT_BODY_BIND_V1_R2_MOBILE_390x844_2026-09-17.png');
  ...
  if (failed.length) throw new Error(`Cat Body Bind V1 R2 assertions failed: ${failed.map(([key]) => key).join(', ')}`);
  ...
  await browser.close();
  const reportPath = path.join(qaDir, 'CAT_BODY_BIND_V1_R2_BROWSER_QA_2026-09-17.json');
