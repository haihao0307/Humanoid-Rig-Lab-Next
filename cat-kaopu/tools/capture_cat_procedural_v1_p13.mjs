...
const url = process.env.CAT_PROCEDURAL_P13_URL ?? 'http://127.0.0.1:4173/cat-kaopu/procedural-cat-v1/index.html';
...
const report = {
  schema: 'cat_kaopu/procedural_cat_p1_3_browser_qa@1.0',
  buildId: 'cat-procedural-body-v1-p1-3-mass-20260918',
  url,
  ready: null,
  pageErrors: [],
  consoleErrors: [],
...
try {
  ...
  const capture = async (view, suffix = view) => {
    ...
    const filename = `CAT_PROCEDURAL_V1_P1_3_${suffix.toUpperCase().replaceAll('-', '_')}_2026-09-18.png`;
  ...
  variantDna.meta.id = 'grey-tabby-a-p1-3-qa-variant';
  variantDna.meta.revision += 1;
  variantDna.global.overallBulk *= 1.025;
  variantDna.torso.thoraxWidth *= 1.020;
  variantDna.torso.lumbarWidth *= 0.990;
  ...
  report.assertions.neckRatioBounded = dna.neck.length / dna.global.bodyLength < 0.13;
  ...
  report.assertions.tailSamplingRaised = dna.tail.segments >= 24;
  ...
  const mobilePath = path.join(qaDir, 'CAT_PROCEDURAL_V1_P1_3_MOBILE_390x844_2026-09-18.png');
  ...
  await browser.close();
  fs.writeFileSync(path.join(qaDir, 'CAT_PROCEDURAL_V1_P1_3_BROWSER_QA_2026-09-18.json'), `${JSON.stringify(report, null, 2)}\n`, 'utf8');
}
