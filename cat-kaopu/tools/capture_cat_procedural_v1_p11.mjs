...
const url = process.env.CAT_PROCEDURAL_P11_URL ?? 'http://127.0.0.1:4173/cat-kaopu/procedural-cat-v1/index.html';
...
const report = {
  schema: 'cat_kaopu/procedural_cat_p1_1_browser_qa@1.0',
  buildId: 'cat-procedural-body-v1-p1-1-morphology-20260918',
  url,
  ready: null,
  pageErrors: [],
  consoleErrors: [],
...
try {
  ...
  const capture = async (view, suffix = view) => {
    ...
    const filename = `CAT_PROCEDURAL_V1_P1_1_${suffix.toUpperCase().replaceAll('-', '_')}_2026-09-18.png`;
  ...
  variantDna.meta.id = 'grey-tabby-a-p1-1-qa-variant';
  variantDna.meta.revision += 1;
  variantDna.global.overallBulk *= 1.04;
  variantDna.torso.thoraxWidth *= 1.035;
  variantDna.torso.lumbarWidth *= 0.98;
  variantDna.head.earTiltDeg += 3;
  variantDna.tail.lateral += 0.018;
  ...
  report.assertions.authoringResolutionRaised = report.baselineMetrics.resolution >= 112;
  ...
  report.assertions.neutralSurfaceCaptured = report.screenshots['quarter-front-neutral'].bytes > 10_000;
  ...
  const mobilePath = path.join(qaDir, 'CAT_PROCEDURAL_V1_P1_1_MOBILE_390x844_2026-09-18.png');
  ...
  await browser.close();
  fs.writeFileSync(path.join(qaDir, 'CAT_PROCEDURAL_V1_P1_1_BROWSER_QA_2026-09-18.json'), `${JSON.stringify(report, null, 2)}\n`, 'utf8');
}
