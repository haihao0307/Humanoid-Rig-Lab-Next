...
const url = process.env.CAT_PROCEDURAL_P14_URL ?? 'http://127.0.0.1:4173/cat-kaopu/procedural-cat-v1/index.html';
...
const report = {
  schema: 'cat_kaopu/procedural_cat_p1_4_browser_qa@1.0',
  buildId: 'cat-procedural-body-v1-p1-4-joint-carrier-20260918',
  url,
  ready: null,
  pageErrors: [],
  consoleErrors: [],
...
try {
  ...
  report.massProfile = await page.evaluate(() => window.__CAT_PROCEDURAL_MASS__ ?? null);
  ...
  const capture = async (view, suffix = view) => {
    ...
    const filename = `CAT_PROCEDURAL_V1_P1_4_${suffix.toUpperCase().replaceAll('-', '_')}_2026-09-18.png`;
  ...
  variantDna.meta.id = 'grey-tabby-a-p1-4-qa-variant';
  variantDna.meta.revision += 1;
  variantDna.global.overallBulk *= 1.020;
  variantDna.torso.thoraxWidth *= 1.018;
  variantDna.torso.lumbarWidth *= 0.992;
  variantDna.head.earTiltDeg += 3;
  variantDna.tail.lateral += 0.012;
  ...
  report.assertions.massProfile = report.massProfile === 'cat-procedural-body-v1-p1-3-mass-20260918';
  report.assertions.jointCarrierProfile = report.jointCarrierProfile === 'cat-procedural-body-v1-p1-4-joint-carrier-20260918';
  ...
  report.assertions.shoulderEmbeddedBelowDorsalSurface = report.baselineMetrics.shoulderJointHeightM < dna.global.shoulderHeight - 0.03;
  report.assertions.hipEmbeddedBelowDorsalSurface = report.baselineMetrics.hipJointHeightM < dna.global.hipHeight - 0.03;
  report.assertions.noForeReachCorrection = report.baselineMetrics.foreReachCorrectionM < 1e-8;
  report.assertions.noHindReachCorrection = report.baselineMetrics.hindReachCorrectionM < 1e-8;
  report.assertions.massRhythm = report.baselineMetrics.thoraxLumbarRatio > 1.05 && report.baselineMetrics.thoraxLumbarRatio < 1.25 && report.baselineMetrics.pelvisLumbarRatio > 1.02 && report.baselineMetrics.pelvisLumbarRatio < 1.18;
  report.assertions.compactNeck = dna.neck.length / dna.global.bodyLength < 0.12;
  report.assertions.compactMuzzle = dna.head.muzzleLength / dna.head.cranialLength < 0.19;
  ...
  report.assertions.tailSamplingRaised = dna.tail.segments >= 28;
  ...
  report.assertions.skeletonEvidenceCaptured = report.screenshots['quarter-front-skeleton'].bytes > 10_000 && report.screenshots['left-skeleton'].bytes > 10_000;
  report.assertions.neutralSurfaceCaptured = report.screenshots['quarter-front-neutral'].bytes > 10_000 && report.screenshots['left-neutral'].bytes > 10_000 && report.screenshots['top-neutral'].bytes > 10_000;
  ...
  const mobilePath = path.join(qaDir, 'CAT_PROCEDURAL_V1_P1_4_MOBILE_390x844_2026-09-18.png');
  ...
  await browser.close();
  fs.writeFileSync(path.join(qaDir, 'CAT_PROCEDURAL_V1_P1_4_BROWSER_QA_2026-09-18.json'), `${JSON.stringify(report, null, 2)}\n`, 'utf8');
}
