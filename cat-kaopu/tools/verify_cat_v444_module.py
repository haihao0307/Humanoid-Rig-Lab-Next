...
def main() -> None:
    ...
    assert current["currentVersion"] == "V4.44"
    ...
    assert "__CAT_V444_READY__" in output
    assert "__CAT_V444_PERIORBITAL_GEOMETRY__" in output
    assert "buildContinuousPeriorbital" in output
    assert "const periVS=`#version 300 es" in output
    assert "const periFS=`#version 300 es" in output
    assert "gl.drawElements(gl.TRIANGLES,periMesh.idx.length" in output
    assert "continuousPeriorbitalPatch:true" in output
    assert "geometricEyelidVolume:false" in output
    ...
    print("periorbital pieces: 2")
    print("periorbital vertices: 1170")
    print("periorbital triangles: 2048")
