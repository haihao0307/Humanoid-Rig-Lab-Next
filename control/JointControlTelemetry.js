...
(function(root,factory){const api=factory();if(typeof module==='object'&&module.exports)module.exports=api;else root.JarvisJointControl=api;})(typeof globalThis!=='undefined'?globalThis:this,()=>{
 ...
 class Telemetry{
  constructor(api){this.api=api;this.before=new Map();this.previous=new Map();this.peak=new Map();this.ticket=null;this.latest=null;this.frames=0;this.contractValue=this.contract();}
  contract(){const a=this.api(),h=a.human;return{schema:'jarvis/human_control_contract@1.0',bodyId:'humanoid',bodyVersion:a.version,poseAuthority:'Human.finalPose',units:'metre',rotation:'local_xyzw_quaternion',jointIds:h.joints.map(j=>j.id),jointCount:h.joints.length,hierarchy:h.joints.map(j=>({id:j.id,parentId:j.parent?.id||null,bindLocalPosition:[...j.bind]})),forbiddenWrites:['bindLocalPosition','boneLength','parentId','scale','geometry','material'],directAngleCommands:!!a.jointControl,angleContract:a.jointControl?'jarvis/joint_goal@1.1':null,forceDynamics:false};}
  begin(ticket,step){this.ticket={ticketId:ticket.ticketId,inputId:ticket.inputId||null,planId:ticket.planId,stepId:ticket.stepId,generation:ticket.generation,step:copy(step),goal:{schema:'jarvis/motion_goal@1.0',type:step.type,objectId:step.objectId||null,targetId:step.targetId||null,relation:step.relation||null,referenceFrame:step.referenceFrame||'world',duration:step.duration||null,solverOwner:'Human.finalPose',fixedBoneLengths:true,hardJointROM:true}};this.before.clear();this.peak.clear();this.frames=0;for(const j of this.api().human.joints)this.before.set(j.id,[...j.q]);return this.sample(true);}
  sample(full=false){const a=this.api(),h=a.human;this.frames++;
  ...
  view(full=false){if(!this.latest)return null;const value=copy(this.latest);if(!full)value.joints.sort((a,b)=>b.peakFromStepStartDeg-a.peakFromStepStartDeg),value.joints=value.joints.slice(0,16);return value;}
 }
