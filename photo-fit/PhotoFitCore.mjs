...
function measureView(input,{mode,view,widthPx,heightPx,knownHeightM=null}){
  ...
  if(view==='front'){
    ...
    if(mode==='full'){
      ...
      m.knownHeightM=knownHeightM==null?measure(null,0,[],'unknown'):measure(Number(knownHeightM),1,['user:knownHeightM'],'manual');
...
export function buildShapeCandidate(measurement,shapeReport,{gain=.75,manual={}}={}){
  ...
  const arm=avg([f.armLeftWidthByHeight?.value,f.armRightWidthByHeight?.value].filter(Number.isFinite)),leg=avg([f.thighLeftWidthByHeight?.value,f.thighRightWidthByHeight?.value].filter(Number.isFinite));if(Number.isFinite(arm))putShape(out,evidence,warnings,'armFullness',base.armFullness+Math.log(arm/((metrics.armRadiusM*2)/stature))*gain,'front.armWidthByHeight',.7);if(Number.isFinite(leg))putShape(out,evidence,warnings,'legFullness',base.legFullness+Math.log(leg/((metrics.legRadiusM*2)/stature))*gain,'front.thighWidthByHeight',.7);
...
export function candidateDiff(basePreset,candidate){return{shape:Object.fromEntries(SHAPE_PARAMETER_SPECS.map(s=>[s.id,{before:basePreset.shape[s.id],after:candidate.shape.shape[s.id],delta:candidate.shape.shape[s.id]-basePreset.shape[s.id]}])),face:candidate.face.face.offsetsMm};}
