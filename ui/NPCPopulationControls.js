...
function installNPCPopulationControls(lab,population){
 ...
 function taskStatus(row){
  const b=row.behavior;if(row.error||b.error)return'已阻断 · '+(row.error||b.error);if(row.resource?.mode==='circulating')return(row.resource.conflict?.kind==='turn'?'较早申请优先':'资源占用')+' · 正在改走备用路线后重试\n冲突：'+(row.resource.conflict?.id||'共享资源');
