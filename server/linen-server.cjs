...
const root=path.resolve(__dirname,'..'),port=Number(process.env.LINEN_PORT||8792);
...
http.createServer((req,res)=>{
 let file;try{const route=decodeURIComponent(new URL(req.url,'http://localhost').pathname);file=path.resolve(root,'.'+(route==='/'?'/linen.html':route));}catch{res.writeHead(400);return res.end();}
...
}).listen(port,'127.0.0.1',()=>console.log('Linen skirt workbench: http://127.0.0.1:'+port+'/linen.html?linen=1'));
