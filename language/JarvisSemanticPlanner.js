...
const SKILLS=Object.freeze({joint_pose:'pose_arm',walk:'walk_to_region',turn:'turn_in_place',carry:'carry_object',push:'push_object',sit:'sit_ground',lie:'lie_ground',stand:'stand_up',greet:'greet',wave:'wave',salute:'salute'});
const LABEL={walk:'走到',turn:'转向',carry:'搬运',push:'推动',sit:'坐在地上',lie:'躺下',stand:'起身',greet:'打招呼',wave:'挥手',salute:'敬礼',wait:'等待',observe:'读取场景'};
...
function distanceNumber(text){
 const value=String(text).replace(/两/g,'二');if(value==='半')return .5;if(/^\d+(?:\.\d+)?$/.test(value))return Number(value);
 const digits='零一二三四五六七八九',parts=value.split('点');let total=0,current=0;
 for(const c of parts[0]){if(digits.includes(c))current=digits.indexOf(c);else if(c==='十'||c==='百'){total+=(current||1)*(c==='十'?10:100);current=0;}else return NaN;}
 const fraction=parts[1];return total+current+(fraction?Number('0.'+[...fraction].map(c=>digits.indexOf(c)).join('')):0);
}
function parseRelativeWalk(text){
 const match=String(text).replace(/\s+/g,'').match(/^(?:(?:向|往|朝)(前|后|左|右)(?:方|面|边)?(?:走|行走|移动|前进)?|(?:向)?(前进|后退)|(?:走|行走))(\d+(?:\.\d+)?|[零一二两三四五六七八九十百点]+|半)(米|公尺|m|厘米|cm)$/i);
 if(!match)return null;const direction=({前:'forward',后:'backward',左:'left',右:'right',前进:'forward',后退:'backward'})[match[1]||match[2]]||'forward';
 return {type:'walk',direction,distanceM:distanceNumber(match[3])*(/^(厘米|cm)$/i.test(match[4])?.01:1),referenceFrame:'self'};
}
...
function describeStep(s,w){if(s.type==='turn')return s.headingDeg!=null?'面向场景 '+s.headingDeg+'°':(s.angleDeg<0?'向左':'向右')+'转 '+Math.abs(s.angleDeg)+'°';if(s.type==='joint_pose')return Joint.describe(s);if(s.type==='walk'&&s.direction)return`${s.direction==='forward'?'向前':({backward:'转身向后',left:'转向左侧',right:'转向右侧'})[s.direction]}走 ${s.distanceM} 米（以这一步开始时的朝向为准）`;const all=[...worldData(w).objects,...worldData(w).zones],name=id=>{const e=all.find(e=>e.id===id);return e?`${e.name}（${id}）`:id};const relation={inside:'内',near:'旁边',left:'左侧',right:'右侧',front:'前侧',behind:'后侧'}[s.relation]||'';if(s.type==='wait')return`等待 ${s.duration} 秒`;if(s.type==='observe')return'读取当前场景并汇报';return`${LABEL[s.type]||s.type}${s.objectId?' '+name(s.objectId):''}${s.targetId?(s.objectId?' 到 ':' ')+name(s.targetId)+relation:''}${s.duration?'，持续 '+s.duration+' 秒':''}`}
...
function validatePlan(plan,world,capabilities){
 ...
 if(n.kind!=='action'){err('仅允许语义动作或条件节点');continue}keys(n,['kind','id','step','label','source','status']);const s=n.step||{};if(s.type==='joint_pose'){if(plan.constraints?.forbiddenActions?.includes(s.type))err('关节动作与本计划禁止动作冲突');const jv=Joint.validate(s);errors.push(...jv.errors);if(!available.has('pose_arm'))err('身体尚未声明肩肘目标能力');continue;}keys(s,['type','objectId','targetId','relation','referenceFrame','duration','direction','distanceM','angleDeg','headingDeg']);if(!Object.hasOwn(SKILLS,s.type)&&!['wait','observe'].includes(s.type))err('身体没有该动作：'+s.type);
 if(SKILLS[s.type]&&!available.has(SKILLS[s.type]))err(`已识别“${LABEL[s.type]}”，但当前动作尚未接入；本轮没有开始执行。`);
 if(s.type==='turn'){const v=s.angleDeg??s.headingDeg;if(!Number.isFinite(v)||Math.abs(v)>360||(s.angleDeg!=null&&s.headingDeg!=null))err('转向需要 -360 至 360 度内的单一角度');}else if(s.angleDeg!=null||s.headingDeg!=null)err('角度只用于转向');
 const relative=s.type==='walk'&&s.direction!=null;
 if(relative){if(!['forward','backward','left','right'].includes(s.direction)||!Number.isFinite(s.distanceM)||s.distanceM<.1||s.distanceM>100||s.referenceFrame!=='self'||s.targetId||s.objectId||s.relation)err('相对行走需要一个方向和 0.1 至 100 米距离，不能同时指定目标物体');}
 else if(s.direction!=null||s.distanceM!=null)err('方向和距离只用于相对行走');
 if(['walk','carry','push'].includes(s.type)&&!relative&&!all.some(e=>e.id===s.targetId))err('目标不存在：'+s.targetId);
 ...
 compile(input,world,capabilities,options={}){
  ...
  const parseSequence=(raw,depth=0)=>{
   ...
   for(let chunk of chunks){const againModifier=/^(?:请|帮我|把|将|先|你)*再/.test(chunk);chunk=chunk.replace(/^(?:请|麻烦你|麻烦|帮我|帮忙|能不能|可以帮我|能否帮我|可以|能否|你能|你|先|再|并且|并|把|将|把所有|把全部)+/g,'').replace(/(?:好吗|好不好|可以吗|行吗|吧|一下|呢|啊)$/g,'').trim();if(!chunk)continue;
    ...
    const turn=chunk.match(/^(?:原地)?(?:向|往)?(左|右|后)(?:转|转身|转向)([零一二两三四五六七八九十百点\d.]+)?(?:度|°)?$/);if(turn){const value=turn[2]?distanceNumber(turn[2]):turn[1]==='后'?180:90;out.push(action({type:'turn',angleDeg:turn[1]==='左'?-value:value}));continue;}
    const relativeWalk=parseRelativeWalk(chunk);if(relativeWalk){out.push(action(relativeWalk));if(relativeWalk.direction!=='forward')plan.assumptions.push('人物先转向指定方向，再使用步行采集动作行走。');continue;}
