...
function parseMeasurements(text) {
  const number = '(\\d+(?:\\.\\d+)?|零|一|两|二|三|四|五|六|七|八|九|十|zero|one|two|three|four|five|six|seven|eight|nine|ten)';
...
function parseNumber(value) {
  ...
  return Number.isFinite(numeric) ? numeric : NUMBER_WORDS[String(value).toLowerCase()] ?? null;
}
