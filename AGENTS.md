...
面部工作前先读 [docs/face-knowledge/README.md](docs/face-knowledge/README.md) 及对应模块的来源、失败记录。坚持自主程序化，只学习结构与方法，不搬入他人的模型、扫描、图片贴图或烘焙资产。结束时更新已验证事实、仍失败的样式、源码所有者、证据的时间与构建哈希；明确区分文献结论、代码/数值检查、实际运行观察和待验证假设。旧截图和文件测试不能证明新构建已达到视觉目标。
