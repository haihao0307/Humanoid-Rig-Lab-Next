...
源资产包含原生 `JOINTS_0`、`WEIGHTS_0`、24 组逆绑定矩阵和单一 Three.js `SkinnedMesh`。运行时以追加方式扩展为 67 关节变形调色板，覆盖扭转、肩胛和手指链；源 GLB 及旧 24 关节动作仍保持兼容。场景级单表皮守卫会持续检查历史静态人体、程序化人体和重复 SkinnedMesh，人物拾取直接使用同一张精细网格。

正式视口采用 GPU 线性蒙皮，并在蒙皮输入端应用肩、肘、髋、膝八区稀疏姿势修正；CPU DQS 只作为质量参考。当前绑定仍属于过渡性权重，元数据标记为 `productionReady: false`。生产发布仍需获得许可明确的专业权重和雕刻级姿势修正形变。
...
1. 过渡性蒙皮权重已经验证原生 SkinnedMesh、67 关节运行时扩展和稀疏姿势修正管线；专业发布仍需许可明确的权重与雕刻级姿势修正形变。
...
5. 新增的扭转、手指、眼睛、下颌和肩胛节点已进入骨架、姿势、动画和导出拓扑；运行时已把扭转、手指和肩胛追加到变形调色板，但源 SMPL 资产仍是 24 骨过渡绑定，生产级精细表面形变仍需要后续重新绑定权重。
