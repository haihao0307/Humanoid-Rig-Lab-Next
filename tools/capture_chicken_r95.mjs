...
const checks = {
  ...
  patchLoaded: runtime?.patch?.version === 'V4.6_R9.5_HEAD_CARRIER_CANDIDATE',
  ...
  lowerBodyGuardHeld: typeof audit?.nonHeadMoved === 'number' && audit.nonHeadMoved < 16,
  stationXMonotonic: audit?.stationXMonotonic === true,
  boundedDisplacement: typeof audit?.maxDisplacement === 'number' && audit.maxDisplacement > 0 && audit.maxDisplacement < 0.13,
  crownBounded: typeof profile?.crownTop === 'number' && profile.crownTop > 1.00 && profile.crownTop < 1.03,
  headWidthBounded: typeof profile?.posteriorWidth === 'number' && profile.posteriorWidth > 0.15 && profile.posteriorWidth < 0.18,
  cheekWidthBounded: typeof profile?.cheekWidth === 'number' && profile.cheekWidth > 0.14 && profile.cheekWidth < 0.17,
  shortenedBill: typeof profile?.mappedBillTip === 'number' && profile.mappedBillTip > 0.475 && profile.mappedBillTip < 0.495,
