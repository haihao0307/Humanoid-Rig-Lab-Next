#!/usr/bin/env python3
"""Standing hand-gesture proof using the real Compact body and direct Renderer."""
...
def load_real_compact(driver, url: str) -> dict[str, Any]:
    ...
    real = proof.wait_until(ready, 360, "真实 Compact 人物没有完成重建", .25)
    return {**state, "realCompact": real}


def isolate_character(driver) -> dict[str, Any]:
    return proof.body_js(
        ...
        if(!compact)throw Error('缺少当前人物 Compact 表面');
        ...
        lab.focus('body');lab.inspectBody('front');
        for(const el of document.querySelectorAll('[class*=label],.object-label,.zone-label'))el.style.visibility='hidden';
        return{mode:'direct-render-real-compact-gesture-v1',hiddenSceneItems,hiddenOtherActors,
          activeId:active?.id||null,compact:{enabled:compact.enabled!==false,chunks:compact.chunks?.length||0,
            triangles:compact.report?.triangles||0,boundToActive:compact.boundHuman===lab.human}};
        """,
    )


proof.load = load_real_compact
proof.capture = lambda driver, out_dir, stem, label, names, subtitle, completed=True, error=None: capture_direct(
    proof, driver, out_dir, stem, label, names, subtitle, completed, error
)
gesture.isolate_character = isolate_character
