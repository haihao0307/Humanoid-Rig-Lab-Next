...
replacements = [
    (
        "slotPoint:null,slotIndex:null};this.sync();",
        "slotPoint:null,slotIndex:null,advancePoint:null,advanceRouteIndex:-1};this.sync();",
    ),
    (
        "Object.assign(locomotion.traffic,{slotKey:key,slotTaskKey:taskKey,slotPoint:[...point],slotIndex:index});",
        "Object.assign(locomotion.traffic,{slotKey:key,slotTaskKey:taskKey,slotPoint:[...point],slotIndex:index,advancePoint:null,advanceRouteIndex:-1});",
    ),
    (
        "originalTarget:[...goal],lastPlanAtS:a.time,nextPlanAtS:a.time+TRAFFIC_AVOIDANCE.planCooldownS,lastError:null});this.traffic.replans++;",
        "originalTarget:[...goal],lastPlanAtS:a.time,nextPlanAtS:a.time+TRAFFIC_AVOIDANCE.planCooldownS,lastError:null,advancePoint:null,advanceRouteIndex:-1});this.traffic.replans++;",
    ),
    (
        "originalTarget:[...current],lastPlanAtS:a.time,nextPlanAtS:a.time+TRAFFIC_AVOIDANCE.planCooldownS,lastError:null});this.traffic.detours++;",
        "originalTarget:[...current],lastPlanAtS:a.time,nextPlanAtS:a.time+TRAFFIC_AVOIDANCE.planCooldownS,lastError:null,advancePoint:null,advanceRouteIndex:-1});this.traffic.detours++;",
    ),
    (
        "originalTarget:[...target],lastPlanAtS:a.time,nextPlanAtS:a.time+TRAFFIC_AVOIDANCE.planCooldownS,lastError:null});this.traffic.retreats++;",
        "originalTarget:[...target],lastPlanAtS:a.time,nextPlanAtS:a.time+TRAFFIC_AVOIDANCE.planCooldownS,lastError:null,advancePoint:null,advanceRouteIndex:-1});this.traffic.retreats++;",
    ),
]

for old, new in replacements:
    if text.count(old) != 1:
        raise SystemExit(f'expected exactly one source site: {old[:80]}')
...
if text.count(marker) != 1:
    raise SystemExit('expected one prepareTrafficTarget insertion marker')
...
if text.count(old) != 1:
    raise SystemExit('expected one traffic planning sequence')
text = text.replace(old, new)

path.write_text(text, encoding='utf-8', newline='\n')
print('rolling-target patch applied')
