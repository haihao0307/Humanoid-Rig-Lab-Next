...
const loadedBox={id:'neighbor',shape:'box',p:[0,0,0],w:.3,d:.3,collidable:true},loadedWorld=makeWorld([loadedBox]);
loadedWorld.collision=(p,r)=>Math.hypot(Math.max(Math.abs(p[0])-.15,0),Math.max(Math.abs(p[2])-.15,0))<r+.06;
const loadedAgent=makeAgent('npc-loaded',makeHuman(),loadedWorld,[2,0,0]);loadedAgent.pos=[.55,0,0];loadedAgent.yaw=Math.PI/2;loadedAgent.skill={type:'carry'};loadedAgent.held={id:'held'};
const loadedLoc=new api.NaturalLocomotion(loadedAgent);loadedAgent.locomotion=loadedLoc;
const loadedResult=run(loadedAgent,loadedLoc,1800);
assert(loadedResult.completed,'the real motion kernel must leave an expanded carrying margin');
assert.equal(loadedLoc.traffic.recoveries,0);assert.equal(loadedLoc.engine.state.metrics.rejected,0);assert(loadedResult.maxFootErrorM<1e-7);
console.log(JSON.stringify({loadedMarginExit:true,completed:true,recoveries:loadedLoc.traffic.recoveries,maxFootErrorM:loadedResult.maxFootErrorM}));
