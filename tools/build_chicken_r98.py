#!/usr/bin/env python3
"""Build Chicken V4.6 R9.8 connected head with collapsed legacy bill."""
...
base = r96.base
base.OUTPUT = base.ROOT / "CHICKEN_V46_R9_8_HEAD_COLLAPSED_BILL.html"
base.PARAMS = base.ROOT / "data" / "CHICKEN_R98_HEAD_COLLAPSED_BILL_PARAMETERS.json"
base.STATIC_QA = base.ROOT / "qa" / "CHICKEN_R98_STATIC_QA.json"
base.BROWSER_QA = base.ROOT / "qa" / "CHICKEN_R98_BROWSER_QA.json"
base.MANIFEST = base.ROOT / "BUILD_MANIFEST_R98.json"
base.REVIEW_BOARD = base.ROOT / "evidence" / "r98" / "R98_REVIEW_BOARD.html"
base.PATCH_MARKER = "CHICKEN_R98_HEAD_BILL_PATCH"
base.JS_OVERRIDE = Path(__file__).with_name("chicken_r98_override.js").read_text(encoding="utf-8").strip()
base.VISIBLE_REPLACEMENTS = {
    "<title>Chicken R9.1 · Final Silhouette & Head Top Gate</title>": "<title>Chicken R9.8 · Connected Head and Collapsed Legacy Bill</title>",
    "鸡 · R9.1 · 最终轮廓与头顶收敛": "鸡 · R9.8 · 连续头部与内收旧喙候选",
    "冻结 R9 作为前版 · 重塑头顶、单冠连续叶片与面部材质边界 · 继续检查全身轮廓": "从冻结 R9.1 重建 · 头部仅做有界上颅/侧颊调整，旧喙内收到闭合短喙内部",
    "后：R9.1 头顶与轮廓": "后：R9.8 连续头部与短喙",
...
def write_parameters() -> None:
    base.write_json(base.PARAMS, {
        ...
        "version": "V4.6_R9.8",
        "status": "connected_head_collapsed_legacy_bill_candidate_not_anatomical_truth",
        ...
        "source": {
            ...
            "rejected_predecessors": [
                ...
                "R9.7 triangle-cut spikes and open face-root artifact"
            ],
        },
        "carrier": {
            "method": "connected dorsal cranium with side-local cheek/jaw field; no lower-neck movement",
            "x_domain": [0.248, 0.438],
            "legacy_bill_collapse": {"source_after_x": 0.432, "target_x_range": [0.427, 0.429], "radial_scale": 0.055},
            "compact_bill": {"root_x": 0.426, "tip_x": 0.484, "sections": 34, "angular_samples": 36, "closed_root": True, "closed_tip": True},
        },
        "local_modules": {
            "comb": "continuous thick five-lobe single comb with embedded root",
            "eyes": "small shallow iris patches without oversized neutral-gray halo",
            "ear_lobes": "suppressed pending a better post-orbital soft-tissue carrier",
            "nostrils": "paired patches on the compact bill",
...
def write_static_qa(build: dict) -> None:
    ...
    base.write_json(base.STATIC_QA, {
        "schema": "life_ecosystem/chicken_r98_static_qa@1.0",
        "version": "V4.6_R9.8_CONNECTED_HEAD_COLLAPSED_BILL_CANDIDATE",
        "checks": checks,
...
def write_state_and_handoff(browser_ok: bool) -> None:
    base.write_json(base.STATE, {
        ...
        "head_status": {"r9_2_through_r9_7_visual_gate_passed": False, "r9_8_candidate_built": True, "r9_8_browser_qa_passed": browser_ok, "manual_visual_acceptance": False},
        ...
        "next_stage": "INDEPENDENT_VISUAL_AUDIT_R98_THEN_BOUNDED_REFINEMENT",
    })
    text = f'''# Chicken V4.6 R9.8 — Connected Head and Collapsed Legacy Bill Candidate
...
R9.8 removes the R9.7 triangle cut. Legacy bill stations are uniformly collapsed inside a smaller closed bill, so no cut-edge spikes are exposed. The head carrier is modified only in dorsal and lateral face bands, with a hard guard on the lower neck and body. Eye, nostril, wattle and comb modules remain compact; the ear-lobe patch stays suppressed until its carrier is credible.
...
'''
    ...
    base.NEXT_PLAN.write_text("""# Next Stage — Chicken R9.8 Independent Visual Audit

1. Compare neutral left/right/front/top/three-quarter views with the approved construction reference.
2. Verify that the legacy-bill collapse is fully hidden and that no cut-edge spikes or face-root holes remain.
3. Check posterior cranium, crown, cheek/jaw support, eye scale, attached wattle and comb root.
...
def write_manifest() -> None:
    ...
    base.write_json(base.MANIFEST, {"schema": "life_ecosystem/build_manifest@1.0", "package": "CHICKEN_V4_6_R9_8_CONNECTED_HEAD_COLLAPSED_BILL_CANDIDATE_2026-09-16", "active_entry": base.OUTPUT.name, "frozen_source": base.SOURCE.name, "frozen_source_sha256": base.EXPECTED_SOURCE_SHA256, "manual_visual_acceptance": False, "files": entries})
