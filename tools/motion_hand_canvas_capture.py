...
def capture_canvas(proof, driver, out_dir: Path, stem: str, label: str,
    ...
    payload = proof.body_js(
        ...
        lab.render();
        const gl=lab.renderer?.gl||canvas.getContext('webgl2');
        gl?.finish?.();
        ...
        return{dataUrl,width:copy.width,height:copy.height,
          drawCalls:lab.renderer?.drawCalls??null,shadowDrawCalls:lab.renderer?.shadowDrawCalls??null,
          compactVisible:lab.compact?.visible===true,compactChunks:lab.compact?.chunks?.length||0};
        """,
    )
    ...
    if image.width < 500 or image.height < 300:
        image.close()
        raise RuntimeError(f"WebGL PNG 尺寸异常：{image.width}x{image.height}")
    c = state.get("canvas", {})
    ...
    if pts:
        ...
        if box[2] - box[0] < 460:
            cx = (box[0] + box[2]) // 2
            box[0], box[2] = max(0, cx - 230), min(image.width, cx + 230)
        if box[3] - box[1] < 390:
            cy = (box[1] + box[3]) // 2
            box[1], box[3] = max(0, cy - 195), min(image.height, cy + 195)
        close = image.crop(tuple(box))
    else:
        w, h = image.size
        close = image.crop((int(w * .18), int(h * .06), int(w * .82), int(h * .9)))
    ...
    return {
        ...
        "state": state, "framebuffer": {k: v for k, v in payload.items() if k != "dataUrl"},
