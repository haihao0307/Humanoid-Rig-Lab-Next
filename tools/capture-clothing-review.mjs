...
try{
 ...
 await page.evaluate(simulated=>{
  ...
  window.clothQA=()=>{
   ...
   gl.transformFeedbackVaryings(program,['worldPoint'],gl.INTERLEAVED_ATTRIBS);gl.linkProgram(program);if(!gl.getProgramParameter(program,gl.LINK_STATUS))throw Error(gl.getProgramInfoLog(program));gl.useProgram(program);
   for(let i=0;i<gl.getProgramParameter(program,gl.ACTIVE_UNIFORMS);i++){
    ...
    const fn=new Map([[gl.FLOAT,'uniform1f'],[gl.INT,'uniform1i'],[gl.BOOL,'uniform1i'],[gl.SAMPLER_2D,'uniform1i'],[gl.INT_VEC2,'uniform2iv'],[gl.INT_VEC3,'uniform3iv'],[gl.FLOAT_VEC3,'uniform3fv'],[gl.FLOAT_VEC4,'uniform4fv']]).get(info.type);
    if(info.type===gl.FLOAT_MAT4)gl.uniformMatrix4fv(loc,false,value);else if(fn){const all=info.size>1?Array.from({length:info.size},(_,j)=>Array.from(gl.getUniform(original,gl.getUniformLocation(original,info.name.replace('[0]','['+j+']'))))).flat():value;gl[fn](loc,all);}else throw Error('uniform '+info.name+' '+info.type);
   }
   gl.bindVertexArray(s.vao);gl.bindBuffer(gl.ARRAY_BUFFER,s.buffers[0]);const count=gl.getBufferParameter(gl.ARRAY_BUFFER,gl.BUFFER_SIZE)/40;
   const raw=new Float32Array(count*10);gl.getBufferSubData(gl.ARRAY_BUFFER,0,raw);
