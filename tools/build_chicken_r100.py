...
def build()->dict:
 ...
 source=SOURCE.read_text(encoding='utf-8');patch=PATCH.read_text(encoding='utf-8').strip()
 ...
 return{'source':str(SOURCE.relative_to(ROOT)),'source_sha256':sha(SOURCE),'patch_sha256':sha(PATCH),'output_sha256':sha(OUTPUT),'source_bytes':SOURCE.stat().st_size,'patch_bytes':PATCH.stat().st_size,'output_bytes':OUTPUT.stat().st_size,'marker_count':output.count(MARKER)}
...
def write_params()->None:
 write_json(PARAMS,{
  ...
  'runtime':{'controller':'runtime/chicken_phase1_npc_controller.mjs','articulated_skin':'runtime/chicken_phase1_articulated_skin.mjs','root_motion_scale':0.42,'fixed_bone_lengths':True,'non_root_scale_forbidden':True},
...
def write_static(build_info:dict)->None:
 checks={'source_exists':SOURCE.exists(),'patch_exists':PATCH.exists(),'single_patch':build_info['marker_count']==1,'output_created':OUTPUT.exists(),'output_larger_than_source':build_info['output_bytes']>build_info['source_bytes'],'controller_exists':(ROOT/'runtime/chicken_phase1_npc_controller.mjs').exists(),'skin_runtime_exists':(ROOT/'runtime/chicken_phase1_articulated_skin.mjs').exists()}
...
def write_manifest()->None:
 paths=[SOURCE,PATCH,OUTPUT,PARAMS,STATIC_QA,BROWSER_QA,REVIEW,ROOT/'runtime/chicken_phase1_npc_controller.mjs',ROOT/'runtime/chicken_phase1_articulated_skin.mjs']+sorted(EVIDENCE.glob('*.png'))
