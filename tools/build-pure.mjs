...
import {bundleMotionLab} from './bundle-motion-lab.mjs';
import {bundlePhysics} from './bundle-physics.mjs';
...
export function assemble(){
 ...
 runtime=runtime.replace('/*__MOTION_LAB_CORE__*/',()=>bundleMotionLab(read));
 runtime=runtime.replace('/*__PHYSICS_ENGINE__*/',()=>bundlePhysics(read));
 const shapeCore=read('reconstruction/shape-contract.mjs').replace(/^export /gm,'')+'\n'+read('reconstruction/shape-deform.mjs').replace(/^import \{normalizeCharacterShape,characterShapeParameterKey\} from '\.\/shape-contract\.mjs';\s*$/m,'').replace(/^export /gm,'');
 if(/\b(?:import|export)\s/.test(shapeCore))throw Error('Unexpected shape module dependency');
 runtime=runtime.replace('/*__CHARACTER_SHAPE_CORE__*/',()=>shapeCore);
 const anatomyCore=read('reconstruction/anatomy-rules.mjs').replace(/^export /gm,'');
 const bindingCore=anatomyCore+'\n'+read('reconstruction/binding.mjs').replace(/^import \{createAnatomyRules\} from '\.\/anatomy-rules\.mjs';\s*$/m,'').replace(/^export \{[^}]+\};\s*$/m,'');
 if(/\b(?:import|export)\s/.test(bindingCore))throw Error('Unexpected binding module dependency');
 runtime=runtime.replace('/*__COMPACT_BINDING_CORE__*/',()=>bindingCore);
 const hairProfileCore=read('reconstruction/hair-profile.mjs').replace(/^export /gm,'');
 runtime=runtime.replace('/*__HAIR_PROFILE_CORE__*/',()=>hairProfileCore);
