...
try{
 ...
 await page.waitForFunction(()=>document.querySelector('#bodyFrame')?.contentWindow?.HumanLab?.population?.list().length>=6,null,{timeout:600000});
 ...
 async function capture(actor,name){
  for(const [view,angle]of [['front',.15],['side',Math.PI/2]]){
   const result=await page.evaluate(({id,angle})=>{
    ...
    return{png,phase:a.agent.phase,posture:a.agent.basic.posture,error:a.agent.error,root:a.agent.pos,footErrorM:a.agent.stats.maxFootPositionErrorM,ground:h.minimumBoneY(),groundCorrectionM:h.motionDriver.report().groundCorrectionM};
   },{id:actor.id,angle});
