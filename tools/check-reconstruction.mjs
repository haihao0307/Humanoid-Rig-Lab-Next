...
export function checkReconstructionSources({root,parse,read,runtime,assert}){
 ...
 check(/includeHair:data\.includeHair===true/.test(worker)&&/function loadCompactSurfaceJob\(quality='preview',includeHair=false,rig=null,job='surface',hairProfile=null,shape=rig\?\.shape\)/.test(bridge)&&/function loadCompactSurface\(\.\.\.args\)/.test(bridge)&&/return loadCompactSurfaceJob\(\.\.\.args\)/.test(bridge)&&/scheduleCompactHair\(window\.HumanLab\.population\.active\)/.test(runtime),'queued startup schedules profile hair after the body and carries every personal shape');
