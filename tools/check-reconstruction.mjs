...
export function checkReconstructionSources({root,parse,read,runtime,assert}){
 ...
 check(/j=>j\.id==='head'/.test(hairRenderer)&&/texelFetch\(compactPalette,ivec2\(0,hairJoint\),0\)/.test(hairRenderer)&&/vec3\(-start\.x,start\.yz\)/.test(hairRenderer)&&hairRenderer.includes('compactFaceIdentity(start,dummyN)')&&bridge.includes('compactFaceIdentity(source,n)'),'hair and body share identity, reflected source coordinates and head motion');
