 ...
 * the original radial material lengths. No source UV, triangle, mass, body,
...
function completeShortsGussetR25(simulation,state,riseReport,options={}){
  ...
  const base={...state.report,sourceBefore,massBefore,closedSeams:closed,junctionReports,centreFit,geometryAtPlacement,placementRadialLimit,stageTransitionInputRiseValid:riseReport?.valid===true,
...
function sr25FitCentreMinimax(anchor,boundary,restRadii,options={}){
  const iterations=options.iterations??220,meanRest=restRadii.reduce((sum,value)=>sum+value,0)/restRadii.length,step=options.initialStepM??Math.max(.004,meanRest*.45);
  if(!sr25Finite3(anchor)||boundary.length!==restRadii.length||!Number.isInteger(iterations)||iterations<16||iterations>1000||!(step>0))throw Error('shorts-r2.5-gusset: invalid centre minimax inputs');
  const objective=point=>{const strains=boundary.map((target,i)=>Math.abs(sr25Distance(point,target)/restRadii[i]-1)),maximum=Math.max(...strains),anchorPenalty=1e-5*(sr25Distance(point,anchor)/meanRest)**2;return {value:maximum+anchorPenalty,maximum,strains};};
  ...
  for(let iteration=0;iteration<iterations;iteration++){
    simplex.sort((a,b)=>a.score.value-b.score.value);const best=simplex[0],worst=simplex[3],secondWorst=simplex[2],centroid=[0,1,2].map(k=>(simplex[0].point[k]+simplex[1].point[k]+simplex[2].point[k])/3);
    const reflected=centroid.map((value,k)=>value+(value-worst.point[k])),r={point:reflected,score:objective(reflected)};
    ...
    else if(r.score.value<secondWorst.score.value)simplex[3]=r;
    ...
    simplex.sort((a,b)=>a.score.value-b.score.value);const diameter=Math.max(...simplex.slice(1).map(item=>sr25Distance(item.point,simplex[0].point)));if(diameter<1e-9)break;
  }
  simplex.sort((a,b)=>a.score.value-b.score.value);return {method:'deterministic_nelder_mead_minimax_original_radial_strain',point:[...simplex[0].point],maximumRadialStrain:simplex[0].score.maximum,radialStrains:[...simplex[0].score.strains],anchor:[...anchor],iterations};
}
