...
uniform sampler2D skirtBinding;
uniform vec3 skirtRestCentre;
uniform int skirtBindingColumns;
uniform bool skirtBoundWaist;
out vec3 W,N;
...
vec3 waistDelta(vec3 p,float t){
 float angle=atan(p.x-skirtRestCentre.x,p.z-skirtRestCentre.z),column=mod(angle/6.28318530718,1.)*float(skirtBindingColumns);
 int a=int(floor(column))%skirtBindingColumns,b=(a+1)%skirtBindingColumns,row=t<.07?0:t<.22?1:2;
 float lo=row==0?0.:row==1?.07:.22,hi=row==0?.07:row==1?.22:.4,w=clamp((t-lo)/(hi-lo),0.,1.);
 vec3 x=mix(texelFetch(skirtBinding,ivec2(a,row),0).xyz,texelFetch(skirtBinding,ivec2(b,row),0).xyz,fract(column));
 vec3 y=mix(texelFetch(skirtBinding,ivec2(a,row+1),0).xyz,texelFetch(skirtBinding,ivec2(b,row+1),0).xyz,fract(column));
 return mix(x,y,w)*(1.-smoothstep(.07,.4,t));
}
...
void main(){
 ...
 if(skirtBoundWaist&&detail.x<.4)worldPoint+=waistDelta(p,detail.x);
 ...
 vec3 contactNormal=worldNormal*(dot(normal.xz,p.xz-skirtRestCentre.xz)<0.?-1.:1.);
 ...
 if(detail.x>.07&&detail.y<1.5)for(int iteration=0;iteration<3;iteration++){
  worldPoint=clearThigh(worldPoint,skirtThighs[0],skirtThighs[1],contactNormal);
  worldPoint=clearThigh(worldPoint,skirtThighs[2],skirtThighs[3],contactNormal);
...
class ProceduralLinenSkirt {
 constructor(surface,meshes){
  ...
  const attachmentJoints=new Set(h.joints.flatMap((j,i)=>j.id==='hips'||/^(L[1-5]|T[0-9]+)$/.test(j.id)||j.id.includes('femur')?[i]:[]));
  this.skinBinding=new GarmentSkinBinding(meshes,point,{columns:96,scale:s,allowedJoints:attachmentJoints});this.boundWaist=true;this.restCentre=[cx,top,cz];
  this.poseJointIds=[...new Set([...this.skinBinding.jointIndices.map(i=>h.joints[i].id),'hips','left_femur','left_tibia','right_femur','right_tibia'])];
  ...
  if(values.some(v=>!Number.isFinite(v))||values.length/11>65535)throw Error('Invalid linen geometry');
  const gl=this.gl;this.count=indices.length;this.geometryBytes=values.length*4+indices.length*2;
  this.report={generator:'procedural-linen-skirt@2',simulationEnabled:false,componentSource:{commit:'8b34db7993b27f8cdb40c801a40bf20931d8a416',garmentBlob:'dc2a26171f225c75fd30383bd521d1e3b8e31294',materialBlob:'eae07ce2a18ffd2d1241e97372258b99438bdee8'},visualAcceptance:false,material:LINEN_MATERIAL_SOURCE.material,triangles:indices.length/3,panels:panelCount,panelRanges,blades:0,opaqueCoverage:true,waistY:top,lengthM:length,thicknessM:thickness,geometryBytes:this.geometryBytes,materialCoordinates:'centimeter, independent gore rest coordinates',textureScale:LINEN_TEXTURE_SCALE,hem:'turned geometry',waistband:'separate geometry',clearance:{method:'inherited posed thigh envelope and floor',radiiM:[...this.thighRadii],iterations:3,clothDynamics:false}};
  try{
   ...
   for(const p of [this.main,this.depth])for(const name of ['compactPalette','skirtJoints','skirtTime','skirtScale','skirtColour','skirtThighs[0]','skirtBinding','skirtRestCentre','skirtBindingColumns','skirtBoundWaist'])p.u[name]=gl.getUniformLocation(p.p,name);
   this.bindingTexture=gl.createTexture();gl.activeTexture(gl.TEXTURE0+COMPACT_PALETTE_UNIT+1);gl.bindTexture(gl.TEXTURE_2D,this.bindingTexture);gl.texParameteri(gl.TEXTURE_2D,gl.TEXTURE_MIN_FILTER,gl.NEAREST);gl.texParameteri(gl.TEXTURE_2D,gl.TEXTURE_MAG_FILTER,gl.NEAREST);gl.texParameteri(gl.TEXTURE_2D,gl.TEXTURE_WRAP_S,gl.CLAMP_TO_EDGE);gl.texParameteri(gl.TEXTURE_2D,gl.TEXTURE_WRAP_T,gl.CLAMP_TO_EDGE);gl.texImage2D(gl.TEXTURE_2D,0,gl.RGBA32F,this.skinBinding.columns,this.skinBinding.levels.length,0,gl.RGBA,gl.FLOAT,this.skinBinding.data);gl.activeTexture(gl.TEXTURE0);
 ...
 reset(){}
 setSimulation(enabled){if(enabled)throw Error('亚麻裙尚未通过动态布料验收，当前使用蒙皮与几何避让');}
 update(){
  if(this.disposed)return false;const surface=this.surface;
  const h=surface.boundHuman,palette=surface.palette;
  if(!this.skinTransforms){this.skinTransforms=[];for(const i of new Set([...this.skinBinding.jointIndices,this.joints[0]]))this.skinTransforms[i]={q:new Float64Array(4),d:new Float64Array(4)};}
  for(let i=0;i<this.skinTransforms.length;i++)if(this.skinTransforms[i]){this.skinTransforms[i].q.set(palette.subarray(i*8,i*8+4));this.skinTransforms[i].d.set(palette.subarray(i*8+4,i*8+8));}
  const metrics=this.skinBinding.update(this.skinTransforms,surface.muscleFrames||r2MuscleFrames(h),surface.statureScale,this.joints[0]);this.bindingUpload=true;
  this.report.attachment={method:'same-skin-waist-field',pins:this.skinBinding.pins.length,...metrics,joints:this.skinBinding.jointIndices.map(i=>h.joints[i].id)};
 ...
 draw(depth){
  ...
  gl.activeTexture(gl.TEXTURE0+COMPACT_PALETTE_UNIT+1);gl.bindTexture(gl.TEXTURE_2D,this.bindingTexture);if(this.bindingUpload){gl.texSubImage2D(gl.TEXTURE_2D,0,0,0,this.skinBinding.columns,this.skinBinding.levels.length,gl.RGBA,gl.FLOAT,this.skinBinding.data);this.bindingUpload=false;}
  gl.uniform1i(p.u.skirtBinding,COMPACT_PALETTE_UNIT+1);gl.uniform3fv(p.u.skirtRestCentre,this.restCentre);gl.uniform1i(p.u.skirtBindingColumns,this.skinBinding.columns);gl.uniform1i(p.u.skirtBoundWaist,this.boundWaist?1:0);
 ...
 dispose(){if(this.disposed)return;this.disposed=true;const gl=this.gl;for(const b of this.buffers)gl.deleteBuffer(b);if(this.bindingTexture)gl.deleteTexture(this.bindingTexture);if(this.vao)gl.deleteVertexArray(this.vao);if(this.main)gl.deleteProgram(this.main.p);if(this.depth)gl.deleteProgram(this.depth.p);this.buffers=[];}
}
