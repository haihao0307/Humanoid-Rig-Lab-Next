...
class ShortsCloth {
  constructor(pattern,body,options={}){
    ...
    this.continuousContact=this.options.selfContact?createShortsContinuousContact(this.particles,this.triangleRecords,this.edges,{thickness:this.options.thickness,maxCandidates:this.options.maxSelfCandidates,dofs:this.dofs}):null;
  ...
  _fixedStep(alphaStart=0,alphaEnd=1){
    ...
    for(let sub=0;sub<this.options.substeps;sub++){const alpha=alphaStart+(alphaEnd-alphaStart)*(sub+1)/this.options.substeps,time=this.time+h*(sub+1);this.body?.sample?.(alpha);this.contactEpoch=(this.contactEpoch||0)+1;
      const dampingRate=this.options.handlingDamping!==null&&this.temporarySupports.some(s=>s.active)?this.options.handlingDamping:this.options.damping;
      ...
      this._bodyContact(alpha);this._selfContact(false);
