...
function installShortsControls(lab){
 const query=new URLSearchParams(window.parent.location.search),single=query.has('shorts')||query.has('linen'),placementReview=query.get('shortsPlacement')==='r2.2',leftTubeReview=query.get('shortsStage')==='r2.3-left',stagedReview=placementReview||leftTubeReview;
 const api={studio:single,placementReview,leftTubeReview,focus(view='angle'){
 ...
 panel.innerHTML=(placementReview?'<b>R2.2 九片原布刚性初摆</b>':leftTubeReview?'<b>R2.3 左裤筒成形检查</b>':'<b>布片短裤</b>')+'<span data-status style="margin-left:12px"></span><div><button data-view="angle">全身</button><button data-view="front">正面</button><button data-view="side">侧面</button><button data-view="back">背面</button><button data-view="cloth">近看</button>'+(stagedReview?'':'<button data-action="sew">继续缝制</button><button data-command="向前走0.6米">走路</button><button data-command="坐在地上">坐下</button><button data-command="站起来">站起</button><button data-action="pause">暂停</button>')+'</div>'+(stagedReview?reviewLegend:'');
 ...
 function refreshStatus(){
  ...
  if(leftTubeReview){const report=garment?.leftTubeReport,strain=report?.materialAtCheckpoint?.maxAbsPrincipalStrain;panel.querySelector('[data-status]').textContent=report?.valid?'左裤筒内外缝闭合 · 裤脚环成立 · 当前应变 '+(strain*100).toFixed(2)+'% · 右侧未缝合':'左裤筒检查未通过 · '+(Number.isFinite(strain)?'当前应变 '+(strain*100).toFixed(2)+'%':'等待报告');return;}
